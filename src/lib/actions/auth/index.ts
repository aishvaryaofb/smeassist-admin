...
export async function sendOtp(prevState: any, formData: FormData) {
    ...
    const responseData = await RequestManager.post(`/api/v1/login/send/otp?otpType=MOBILE&msisdn=${mobile}`);
    if (!responseData) {
        return {
            ...prevState,
            errors: null,
...
export async function verifyOtp(prevState: any, formData: FormData) {
    ...
    const responseData = await RequestManager.post(`/api/v1/login/otp?createDummyOrg=false`, payload);
    if (!responseData) {
        return {
            ...prevState,
            errors: null,
